...
GParsPool.withPool {
  ...
  genes.eachParallel { gene ->			
		       //    def i = val.toInteger()
		       //    def x = i.intdiv(diseases.size())
		       //    def y = i % diseases.size()

    def anno = gene.getAnnotations()
    Expando exp = new Expando()
    diseases.each { dis ->
      def anno2 = dis.getAnnotations()
      println "${gene.id}\t${dis.id}\t$anno\t$anno2"
      result[gene][dis] = engine.compare(
            smConfGroupwise,
            smConfPairwise,
	    anno,
	    anno2)
    }		       
    println "Processed $c out of ${genes.size()}."
    c+=1		      
  }
}

def fout = new PrintWriter(new BufferedWriter(
					      //  new FileWriter("./scores_norestriction/Resnik.Extrinsic.simResnik.gene-disease.MGI.txt")))
  new FileWriter("/tmp/abc")))

    result.each { k, v ->
	v.each { g, val ->
	    fout.println("$k\t$g\t$val")
        }
    }
fout.flush()
fout.close()
